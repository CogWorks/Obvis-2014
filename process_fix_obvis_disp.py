    ...
    def assign_object(self,fix):
        if fix.trial_phase == 'find':
            ...
            if fix.aoi != 'c' and self.exp_name != 'experiment-1':
                ...
                fix.image_id = int(self.current_trial.stim_pos[int(fix.aoi[-1])])
...
class TaskDisplay():
    ...
    stimuli_pos = {
        'd0': (940,400) ,'d1': (852,612) ,'d2': (640,700) ,'d3': (427,612) ,'d4': (340,400) ,'d5': (427,187) ,'d6': (640,100),'d7': (852,187) , 'c': (640,400)
    }
    fullscreen = False
    screen = None
...
if __name__ == "__main__":
    ...
    A = DispersionAlgo(args.fix_threshold)
    TD = TaskDisplay()
    #E = PyMouseEvent()
    #E.start()
    if args.algo == 'disp':
